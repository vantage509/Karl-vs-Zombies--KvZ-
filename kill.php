...
if($_POST['submit'] == 'Report Tag') { // Process Tag Report
    $victim = strtoupper(preg_replace("/[^a-z0-9]/i","",$_POST['victim_id']));
    $feed = is_array($_POST['feed']) ? $_POST['feed']: array();
    array_unshift($feed, $_SESSION['id']);//preg_replace("/[^a-z0-9]/i","",)
    ...
    // Loop through fed zombies to check for issues
    ...
    if($err == 0) {
        ...
        // Loop through fed zombies to update feed times
        for($i = 0; $i < sizeof($feed); $i++) { if(strlen($feed[$i]) > 0) {
            ...
            $query = "UPDATE {$table_u} SET feed = TIMESTAMP '{$kill_time}' 
                ...
                AND timediff('{$kill_time}', feed) >= 0 AND active AND state<0;";
    ...
    print "</td></tr></table>";
} else { // Otherwise, show Report Tag form
?>
